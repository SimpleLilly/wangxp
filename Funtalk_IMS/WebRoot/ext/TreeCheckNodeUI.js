...
Ext.extend(Ext.ux.TreeCheckNodeUI, Ext.tree.TreeNodeUI, {
    ...
    renderElements : function(n, a, targetNode, bulkRender){
		...
		//alert(n.attributes.buttons);
		var buttonArr;
		var buttons = new Array();
		if(a.buttons_def !== ''){
			//1${添加},2${修改},3${删除}
			
			//alert(n.attributes.buttons_def);
			buttonArr = a.buttons_def.split(',');
			rightArr = a.buttons.split(',');
			for(i = 0; i < buttonArr.length; i+=1){
				var buttonInfo = new Array(3);
				butt = buttonArr[i].split('${');
				buttonInfo[0] = butt[1].substring(0,butt[1].length - 1);
				buttonInfo[1] = butt[0];
				for(j = 0; j < rightArr.length; j++){
					if(buttonInfo[1] == rightArr[j]){
						buttonInfo[2] = true;
						break;
					}else{
						buttonInfo[2] = false;
				...
				buttons[i] = buttonInfo;
			}
			//for(j = 0; j < buttons.length; j+=1 ){
			//	alert(buttons[j][0]+','+buttons[j][1]+','+buttons[j][2]);
			//}
		}
        //var cb = typeof a.checked == 'boolean';
		var cb = (!this.onlyLeafCheckable || a.leaf);
        var href = a.href ? a.href : Ext.isGecko ? "" : "#";
        var bufArr = ['<li class="x-tree-node"><div ext:tree-node-id="',n.id,'" class="x-tree-node-el x-tree-node-leaf x-unselectable ', a.cls,'" unselectable="on">',
            '<span class="x-tree-node-indent">',this.indentMarkup,"</span>",
            '<img src="', this.emptyIcon, '" class="x-tree-ec-icon x-tree-elbow" />',
            '<img src="', a.icon || this.emptyIcon, '" class="x-tree-node-icon',(a.icon ? " x-tree-node-inline-icon" : ""),(a.iconCls ? " "+a.iconCls : ""),'" unselectable="on" />',
            cb ? ('<input name="check" class="x-tree-node-cb" type="checkbox" value="'+n.id+'"' + (a.checked ? 'checked="checked" />' : '/>')) : '',
            '<a hidefocus="on" class="x-tree-node-anchor" href="',href,'" tabIndex="1" ',
             a.hrefTarget ? ' target="'+a.hrefTarget+'"' : "", '><span unselectable="on">',n.text,"</span></a>&nbsp;&nbsp;&nbsp;&nbsp;"];
			for(j = 0; j < buttons.length; j+=1 ){
				bufArr[bufArr.length] = cb ? (buttons[j][0]+'<input name="check" class="x-tree-node-cb" type="checkbox" value="'+n.id+','+buttons[j][1]+'"' + (buttons[j][2] ? 'checked="checked" />' : '/>')) : '';
			}

            bufArr[bufArr.length] = "</div>";
            bufArr[bufArr.length] = '<ul class="x-tree-node-ct" style="display:none;"></ul>';
            bufArr[bufArr.length] = "</li>";
		var buf = bufArr.join('');
        var nel;
